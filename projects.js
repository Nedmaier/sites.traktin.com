const projects = [
  {
    ...
    link: "aerohunt",
    type: "traveltech",
    typename: "Travel-Tech",
    resolvedissue: 7,
    status: "действующий проект",
    difficulty: 5,
    information: [
      "название: AeroHunt — скринер рыночных цен на авиабилеты",
      "участие в проекте: автор идеи, руководитель продукта, UX/UI и frontend-разработка",
      "статус проекта: действующий, активная разработка",
      "период работы проекта: с октября 2025 г. по настоящее время",
      "сайт проекта: <a href=\"https://aerohunt.ru\" target=\"_blank\" rel=\"noopener noreferrer\">aerohunt.ru</a>",
      "масштаб данных: более 1,29 млн анализируемых вариантов",
      "охват: 2 600+ направлений, 60+ стран, 112 городов, 185 авиакомпаний",
      "активные предложения: более 10 000",
      "полный цикл обновления данных: около 29 минут"
    ],
    logo: "media/aerohunt_logo.png",
    screenshots: [
      "media/aerohunt_architecture.png"
    ],
    description: `
      <p><b>AeroHunt</b> — продукт для анализа рынка авиабилетов, который не просто показывает дешёвые варианты, а рассчитывает справедливую цену по каждому направлению и дате. Система сравнивает текущую стоимость с живой статистикой рынка и показывает реальную скидку относительно ценового тренда.</p>
      <p>Пользователь может выбрать несколько городов вылета, страны, города или регионы прилёта, период поездки и дополнительные условия. Результаты обновляются прямо во время работы сайта, а персональный бот продолжает следить за рынком и отправляет уведомление при появлении подходящего предложения.</p>
      <p>Проект объединяет сайт, Telegram-каналы с автоматическими подборками, персонального ассистента и серверный контур регулярного сканирования, проверки и публикации данных.</p>
    `,
    details: "Скринер справедливой рыночной цены и персональный мониторинг",
    solutions: [
      {
        title: "Расчёт справедливой рыночной цены",
        description: `
          <p><b>Задача:</b> отличать действительно выгодные билеты от маркетинговых «скидок» и случайно дешёвых, но неудобных маршрутов.</p>
          <p><b>Решение:</b> система анализирует исторические и текущие цены, формирует базовую стоимость по направлению и дате, исключает завышенные и неликвидные варианты и вычисляет реальное отклонение от рыночного тренда.</p>
          <p><b>Результат:</b> пользователь видит не абстрактную скидку, а понятную оценку выгоды относительно живого рынка.</p>
        `
      },
      {
        title: "Высокочастотный цикл сканирования и актуализации",
        description: `
          <p><b>Задача:</b> поддерживать десятки тысяч предложений в актуальном состоянии без перегрузки сайта и внешних API.</p>
          <p><b>Решение:</b> реализованы очереди маршрутов, фоновые процессы проверки, разделение данных по месяцам, сжатые JSON-фиды, резервная загрузка и автоматическое скрытие устаревших предложений.</p>
          <p><b>Результат:</b> более 1,29 млн вариантов по 2 600+ направлениям обрабатываются примерно за 29 минут.</p>
        `
      },
      {
        title: "Интерактивный поиск и аналитика цен",
        description: `
          <p><b>Решение:</b> созданы карточный и строковый режимы, сортировка по цене, скидке, дате и странам, календарь минимальных цен, история изменения стоимости и интерактивные графики.</p>
          <p>Если скидочного предложения нет, система ищет минимальную цену в общей базе и позволяет проверить её так же, как обычный офер.</p>
        `
      },
      {
        title: "Избранное и устойчивые live-обновления",
        description: `
          <p>Пользователь может сохранять билеты, менять дату через календарь и отслеживать историю цены. Выбранные предложения обновляются без скачков интерфейса, а пропавшие или устаревшие варианты обрабатываются мягко и предсказуемо.</p>
        `
      },
      {
        title: "Персональные задачи и мультиканальные уведомления",
        description: `
          <p>Разработан REST API для создания, изменения, продления и удаления задач мониторинга. Задача учитывает город вылета, страну или город назначения, даты и максимальную цену.</p>
          <p>Единая модель пользователей поддерживает Telegram, MAX и Web. При появлении подходящего предложения система отправляет push, а после окончания периода предлагает продлить или удалить задачу.</p>
        `
      },
      {
        title: "Автоматические Telegram-каналы по городам",
        description: `
          <p>Лучшие предложения автоматически публикуются в Telegram-форуме с отдельными темами по городам вылета. Система создаёт темы, обновляет и удаляет сообщения, предотвращает дублирование и синхронизирует шаблоны между продуктом и бот-платформой.</p>
        `
      },
      {
        title: "Надёжная production-инфраструктура",
        description: `
          <p>Проект включает административную панель, систему логирования, мониторинг фоновых процессов, резервные сценарии загрузки, автоматическую очистку данных и контроль ошибок. Обновления проходят итерационно с историей версий и проверкой UX на desktop и mobile.</p>
        `
      }
    ]
  },
  {
    ...
    link: "genlab",
    type: "ai",
    typename: "AI и автоматизация контента",
    resolvedissue: 7,
    status: "действующий проект",
    difficulty: 4,
    information: [
      "название: GenLabAI News",
      "участие в проекте: продуктовая концепция, архитектура и развитие платформы",
      "статус проекта: действующий, развитие функциональности",
      "период работы проекта: с февраля 2026 г. по настоящее время",
      "сайт проекта: <a href=\"https://genlabai.ru\" target=\"_blank\" rel=\"noopener noreferrer\">genlabai.ru</a>",
      "продукт: GenLabAI News — цифровая редакция полного цикла",
      "источники: сайты, RSS-ленты и новостные каналы",
      "каналы публикации: сайты, Telegram, MAX и внешние новостные ресурсы",
      "работающие контуры: <a href=\"https://aerohunt.ru/blog/\" target=\"_blank\" rel=\"noopener noreferrer\">AeroHunt Blog</a> и <a href=\"https://genlabai.ru/blog\" target=\"_blank\" rel=\"noopener noreferrer\">GenLabAI Blog</a>"
    ],
    logo: "media/genlabai_logo.png",
    screenshots: [
      "media/aerohunt_architecture.png"
    ],
    description: `
      <p><b>GenLabAI News</b> — цифровая редакция и мультипроектная платформа, которая автоматизирует полный цикл работы с новостным контентом: от мониторинга источников и отбора инфоповодов до создания, адаптации и публикации готовых материалов.</p>
      <p>Система обходит сайты, RSS-ленты и новостные каналы, находит материалы по темам и ключевым фразам и выполняет двухэтапную фильтрацию: проверяет релевантность, отсеивает дубли и информационный шум. Для каждого проекта отдельно задаются редакционная политика, стиль, объём, обязательные блоки, стоп-слова и допустимая степень дополнения исходной информации.</p>
      <p>Один инфоповод превращается в несколько версий: развёрнутую SEO-статью для сайта, сокращённую публикацию для Telegram или MAX и материалы для других площадок. Платформа создаёт заголовки, тексты, изображения, категории, SEO-метаданные и постоянные URL. В production уже работают два независимых контура — туристический блог AeroHunt и AI-блог GenLabAI.</p>
    `,
    details: "Цифровая редакция: от мониторинга источников до мультиканальной публикации",
    solutions: [
      {
        title: "Мониторинг источников и поиск инфоповодов",
        description: `
          <p>Платформа собирает публикации с сайтов, RSS-лент и новостных каналов. Для каждого проекта настраиваются тематика, ключевые фразы, исключения, география и приоритеты источников, поэтому в обработку попадают только потенциально полезные инфоповоды.</p>
        `
      },
      {
        title: "Двухэтапная фильтрация и антидубли",
        description: `
          <p>Сначала материалы проверяются на соответствие тематике проекта, затем проходят повторный контроль на дубли, похожие инфоповоды и информационный шум. Это снижает объём нерелевантного контента до начала затратной AI-обработки.</p>
        `
      },
      {
        title: "Управляемая AI-генерация",
        description: `
          <p>Для каждого проекта задаются тональность, структура, объём, обязательные блоки, стоп-слова и границы допустимого дополнения фактов. AI перерабатывает материал в рамках редакционной политики, сохраняя связь с источником и не выходя за установленные ограничения.</p>
        `
      },
      {
        title: "Адаптация под сайты, Telegram и MAX",
        description: `
          <p>Один инфоповод автоматически преобразуется в разные форматы: подробную статью для сайта, сокращённую публикацию для мессенджера и версии для других новостных площадок. Формат, длина, призыв к действию и оформление подстраиваются под конкретный канал.</p>
        `
      },
      {
        title: "SEO, изображения и автоматическая публикация",
        description: `
          <p>Платформа создаёт заголовок, текст и тематическую обложку, назначает категории, SEO-метаданные и постоянный URL. Через защищённый API материал публикуется автоматически либо поступает на премодерацию; повторная отправка обновляет существующую запись без создания дубля.</p>
        `
      },
      {
        title: "Мультипроектная архитектура",
        description: `
          <p>На одной платформе работают независимые контентные контуры с разными тематиками, источниками и правилами: туристический блог AeroHunt и собственный AI-блог GenLabAI. Новый проект подключается настройкой профиля бренда, фильтров, источников и каналов публикации без разработки отдельной системы.</p>
        `
      },
      {
        title: "Production-контроль и интеграции",
        description: `
          <p>Статусы обработки, запросы, ошибки и результаты публикации фиксируются в системных логах. Webhook и REST API связывают очередь материалов, AI-обработку, сайты, мессенджеры, XML-фиды и внешние платформы в единый управляемый workflow.</p>
        `
      }
    ]
  },

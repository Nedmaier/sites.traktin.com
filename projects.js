const projects = [
  ...
  },
  {
    ...
    link: "genlab",
    type: "ai",
    typename: "AI и автоматизация",
    resolvedissue: 5,
    status: "действующий проект",
    difficulty: 4,
    information: [
      "название: GenLabAI",
      "участие в проекте: разработка продукта и интеграция",
      ...
      "назначение: AI-подготовка, обработка и публикация контента",
      "интеграция: production-инфраструктура и блог AeroHunt",
      "инструменты: webhook API, OCR, DocLab, XML-фиды"
    ],
    ...
    description: `
      <p><b>GenLabAI</b> — интеграционный AI-контур для подготовки, обработки и автоматической публикации материалов. Проект связывает инструменты генерации и обработки контента с production-системами, блогом и внешними площадками.</p>
      <p>Через защищённый webhook API система принимает структурированные публикации, обновляет существующие материалы, управляет статусами и метаданными и автоматически пересобирает XML-фиды.</p>
      <p>GenLab развивается как часть общей платформы автоматизации и связан с OCR/DocLab-процессами распознавания и обработки документов.</p>
    `,
    details: "AI-конвейер от обработки материалов до публикации",
    solutions: [
      {
        title: "Защищённый API управления публикациями",
        description: `
          <p>Реализованы операции создания, получения, редактирования и удаления публикаций. API проверяет входные данные, поддерживает статусы, источники, теги и внешние идентификаторы и ограничивает размер выборок.</p>
        `
      },
      {
        title: "Идемпотентная публикация по slug",
        description: `
          <p>При повторной отправке материала система не создаёт дубликат, а находит запись по slug и обновляет её. Такой подход позволяет безопасно повторять запросы и переиздавать материалы после доработки.</p>
        `
      },
      {
        title: "Автоматическое обновление блога и XML-фида",
        description: `
          <p>После создания, изменения или удаления записи автоматически пересобирается XML-фид для внешних площадок. Контент появляется в блоге и связанных каналах без ручного копирования.</p>
        `
      },
      {
        title: "OCR и DocLab-процессы",
        description: `
          <p>В общую платформу включены рабочие контуры распознавания текста и обработки документов. Результаты организуются по продуктам, пользователям и задачам и могут использоваться дальнейшими AI-сценариями.</p>
        `
      },
      {
        title: "Логирование и интеграция с production",
        description: `
          <p>Все входящие запросы, ошибки и результаты публикации фиксируются в системных логах. GenLabAI работает внутри инфраструктуры AeroHunt и использует общие компоненты PHP/MySQL, JSON, фоновые процессы и мониторинг.</p>
        `
      }
    ]
  },
